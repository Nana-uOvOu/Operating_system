...
##### SPOOLing技术
Spooling:假脱机技术.将独享设备虚拟化为假共享设备.
**联机IO**:主机直接控制IO
1. 脱机输入输出:为了缓和高速CPU和低速IO设备的矛盾,脱离主机的控制进行的输入/输出操作
   1. 多道程序系统中,可以利用一道程序来模拟外围输入控制机,*把低速IO上的数据转移到硬盘,*,一道程序来模拟外围输出控制机,*把磁盘数据传送到慢速设备.*脱机输入输出时,IO设备数据处理与CPU数据处理并行,
2. 在联机情况下实现的同时外围操作称为SPOOLing，也叫假脱机。
3. 假脱机实现:输入输出井.使用软件来模拟脱机情况.
![alt text](image-84.png)  
   1. 系统组成:
   > 1. 输入井和输出井：*在磁盘上*,模拟输入/出磁带.
   > 2. 输入/输出缓冲区: *在内存中*.缓和了CPU和磁盘速度不匹配.
   > 3. 输入/输出进程：守护（daemon）进程,模拟脱机时的*外围控制机*.必须要有*多道程序处理*功能
   2. 输入:输入进程控制下,**输入缓冲区(内存中)暂存外部设备的输入数据;然后转存到输入井中**.
   3. 输出:输出进程控制下,**输出缓冲区(内存中)暂存输出井的数据;然后转存到输出设备**.
   ![alt text](image-86.png)
4. 假脱机原理:
   1. **实际IO**:**Spooling程序与外设数据交换**.Spooling从*外设设备*读入数据到输入井,以后需要时再输入到进程;从*进程*读入数据到输出井,以后再输出给外设设备.
   2. **虚拟IO**:**应用程序与Spooling程序进行数据交换**.仅仅是将输入缓冲池数据输入进程/进程输出到输出缓冲池
5. 优点:
   1. 高速虚拟I/O操作;
   2. 设备独享虚拟为共享:本来是各个进程与IO设备交换数据;现在只有Spooling程序交换数据.
6. 共享打印机:
   1. 当用户进程要求打印数据时,os答应请求,并向假脱机管理进程申请.假脱机进程为其在磁盘申请一个输出井缓冲区,*将数据放入输出井*,生成一个打印请求表(写了用户进程,用户数据位置等),挂到假脱机等待队列中.
   2. 打印机空闲时,假脱机进程会将请求表中数据从输出井放入输出缓冲区,并启动打印机进行打印.
### 做题
1. **通道是一种硬件机制**；Spooling、缓冲池、内存覆盖都是软件实现的。
2. 系统为设备分配的不同编号称为**绝对号**
3. *中断IO方式中*，获取输入输出由**中断处理程序完成**，一定不由硬件完成。
4. IO设备向缓冲区输入一个块$T_1$时间，缓冲区读出数据到用户区$T_2$，CPU处理一块$T_3$，则** 单缓冲区处理数据时，第$i$次输入($T_1$)和第$i-1$次读出的处理($T_3$)可以并行，平均一块处理时间为$T_2 + max(T_1,T_3)$**。而双缓冲区，$T_1$和$T_2+T_3$可以并行，所以平均一块处理时间为$max(T_1,T_2+T_3)$
5. 需要缓冲技术的有：鼠标、键盘、磁盘、与总线直接连接的图形卡
6. 单机资源最重要的是**处理器资源**，增大单机资源利用率的最关键技术是：*多道程序设计技术*
7. Spooling必须要有独占设备，**可以加快作业执行速度**。
8. **独占设备静态分配、共享设备动态分配**
9. 打印时，将整数转换为ascii码进行打印是由**用户IO程序完成的，属于用户程序。**
## 磁盘存储器管理
磁盘存储器:*随机读写*共享设备，可移动介质.容量大，速度快.
1. 磁盘存储器要求:合理分配空间、访问效率高，容错能力强。*其性能直接影响文件系统的性能指标*。
2. 磁盘组成结构
   1. ![alt text](image-87.png)
   2. 地址:驱动器号、磁道号(柱面号)、磁头号(盘面号)、扇区号
   3. 磁头分为固定头和移动头
3. 磁盘读取数据:选定的磁头移动到对应磁道,划过磁道扇区,读入数据.使用（柱面号、盘面号、扇区号）来定位
![alt text](image-89.png)
4. 磁盘访问时间:
   1. **寻道时间(主要影响)**：与寻道距离成正比.磁头移动到对应磁道的时间.$T_s = s + m * n$,其中$T_s$是寻道时间,$s$是磁头启动时间(约2ms),$m$为移动一个磁道时间(约0.2ms),$n$为一共需要移动多少磁道.
   2. 旋转延迟时间:*旋转磁盘*,磁头定位到对应扇区时间$T_R = \frac{1}{2} \times \frac{1}{r}$,$\frac{1}{2}$表示求平均,$r$表示转速(转/s),$\frac{1}{r}$表示转一圈时间.
   3. 传输时间:将数据读出的时间.$T_t = \frac{1}{r} \times \frac{b}{N}$,其中$b$是本次需要读取的字节数;$N$是一个磁道的总字节数.
   4. 总时间:$T_a = T_s + \frac{1}{2r} + \frac{b}{rN}$
5. 提高IO速度:硬件性能加强;**调度算法**;缓冲区
6. 操作系统需要对磁盘块进行哪些管理:
   1. 非空闲磁盘块:文件物理结构/分配方式
   > 连续分配;链接分配(显/隐式链接);索引分配
   2. 空闲磁盘块:存储空间管理
### 外存组织方式
1. **物理地址**:磁盘分为多个块.类似分页,一般与内存块/页面大小相同.*内存与硬盘IO以块为单位,每次读入读出一个块*.
2. **逻辑地址**:将文件分为*文件块*,地址格式:逻辑块号、块内地址
![alt text](image-90.png)
##### 分配方式
1. 连续分配方式(支持顺序,随机访问):
   1. *每个文件在磁盘上占有一组连续的块*.
   2. 用户给出逻辑地址(是文件的第几个块)时,os找到文件目录项FCB,计算**物理块号 = 起始块号 + 逻辑块号**
   3. ![alt text](image-91.png)
   4. 连续分配的文件在顺序**读/写时速度最快,但不方便拓展,存储空间利用率低,产生碎片**.,可以用紧凑法,但效率低.
2. 链接分配:隐式链接(**考试中未指明,一律按照隐式**)
   1. 目录表:存放起始,结束块号
   ![alt text](image-92.png)
   ![alt text](image-93.png)
   2. 每个磁盘块都保存了指向下一块的指针(除最后一块),*对用户透明*.读入i个逻辑块,需要访问IO i+1次.
   3. 只支持顺序访问,不支持随机访问;查找效率低,且存储指针也耗时;
   4. 拓展方便,只需要随机找一个物理块,让原来的最后一块存储该块指针即可.没有碎片问题.
3. 链接分配--显式链接:
   1. 目录表:只记录起始块号
   2. **文件分配表 (FAT, File Allocation Table)**:将链接文件的指针放在一张表中.一个磁盘存储一个FAT,开机自动读入内存并常驻.物理块在表中连续存储,物理快号可以隐含
   ![alt text](image-94.png)
   3. *逻辑块号到物理块号转变*:给出逻辑块号i,查找FAT,得到下一块,反复查询即可.**不需要连续IO**,**支持顺序,随机访问**,没有碎片问题,方便扩展
4. 簇(cluster)/部分(partion):存储空间由很多簇组成,每个簇包含多个连续外存单位(例如扇区sector)
   1. 确定大小:
   > 1. 簇大:提高I/O访问性能，减小管理开销
   > 2. 簇较小：簇内的*碎片浪费小*，特别是大量小文件时有利
   2. 是否可变
   > 1. 可变:上限大;IO访问效率高;存储空间管理困难
   > 2. 固定:若较小,则存储空间使用灵活,但IO效率降低,存储空间管理存储开销大
5. 索引分配:
   1. os为*每个文件*建立一张**索引表**,记录了文件的各个逻辑块对应的物理块.有两个表,第一个索引目录表记录了每个文件的索引块(包含了对应文件的索引表所在物理块号),第二个索引表保存了文件的每个逻辑块号(物理块大小固定,可以隐含)对应的物理块号.
   ![alt text](image-95.png)
   2. 进程给出逻辑块号i,os找到对于目录项FCB,找到对应目录,在找到逻辑块号i对应的物理快号.
   3. *支持随机访问,且拓展方便*;但索引表需要占用一定的存储空间
   4. 当一个磁盘块*装不下文件的索引表*(文件过大),解决方法:
   > 1. *链接*方案:将多个物理块链接起来作为索引表.不能随机存取,会有很多IO操作.
   > 2. *多层索引*:类似*多级页表*.各层索引表不能超过一个物理块.
   > 3. 采用K层索引结构，且顶级索引表未调入内存，则访问一个数据块只需要K+1次读磁盘操作.然而对于小文件,仍然需要
   ![alt text](image-96.png)
   > 4. *混合索引*:索引目录表(顶级索引表)中即包含直接索引,也包含一级,多级索引.可以将小文件分配给直接索引,加速访问.
   ![alt text](image-97.png)
6. 总结:
![alt text](image-98.png)
### 做题
1. 在内存设置硬盘缓冲区的目的是**减少硬盘IO次数**
2. 以键盘IO为例，当键盘中断的IO服务程序结束后，键盘输入数据实际存储在**内核数据区**。进程被唤醒进入就绪队列，等待到CPU后才将内核数据区数据放入用户数据。
## 文件存储空间的管理
### 存储空间的划分与初始化
磁盘分区.
1. *存储空间初始化*:将一个分区(卷)划分为目录区(存放文件目录信息(FCB))和文件区
2. *空闲表法(适用于连续分配方式)*:
   1. 空闲表包含了两列,[第一个空闲块号,空闲块数],使用**首次适应、最佳适应、最坏适应等算法**来进行分配空闲空间.
   2. 分配:![alt text](image-99.png)
   3. 回收:![alt text](image-100.png)
   类似动态分区分配,注意*回收时的合并问题*
3. *空闲链表法*:
   1. 空闲盘块链,空闲盘区链
   ![alt text](image-101.png)
   os保存了**链头、链尾指针**
   2. 盘块链:分配K个盘块时,从链头摘下K个空闲盘块使用,并修改链头指针.适用于离散分配
   3. 盘区链:记录了下一个盘块指针,盘区大小;首次适应、最佳适应来找一个适合的盘区.若没有合适的连续空闲块，也可以将*不同盘区的盘块同时分配给一个文件*，修改相应的链指针、盘区大小等数据;回收:回收区和某个空闲盘区相邻，则需要将回收区合并到空闲盘区中;没有盘区相邻,则放在链表尾.
4. *位视图*bitmap:
   1. 字/位 或 行/列号来代表盘块,二进制位为0则空闲,1则占用.$(i,j)$表示盘块号$n * i + j$,$n$为字长
   ![alt text](image-102.png)
   2. 分配:根据文件大小找到一个连续或不连续的0,计算字/位号并**分配盘块**,并设置bit为1.
   3. 回收:根据盘块号计算出字/位号,设置为0;
5. *成组链接法*
   1. 上面的方法不适合大型文件系统,否则空闲(链)表过大.
   2. 文件卷*目录区*中专门用一个磁盘块作为“超级块”，当系统启动时需要将超级块读入内存。并且保证内存与外存中的“超级块”数据一致
   ![alt text](image-103.png)
   3. 空闲盘块的组织:
   > 1. 空闲盘块号栈:存放当前可用的*一组空闲盘块号*，以及栈中尚有的*空闲盘块数*。最多100个
   > 2. *空闲盘块分为若干*组,每组最多100个空闲盘块
   > 3. 将每组含有的盘块总数N和该组的所有盘块号，*记入其前一组的第一个盘块中*，**各组的第一个盘块链成一个链**。
   > 4. 第一组的盘块总数和盘块号，记入空闲盘块栈中，作为当前可分配盘块。
   > 5. 最后一组只有99个盘块号，最后一个盘块号为0，代表链表结束。
   ![alt text](image-104.png)
   ![alt text](image-105.png)
### 磁盘IO调度算法
所有进程的磁盘读写请求是一个随机分布的队列.调度算法**主要目标是减少等待队列的平均寻道时间**.
1. FCFS先来先服务:按照请求队列顺序读取.公平,*适用于低负荷且每次读写连续扇区情况*.
2. 最短寻道时间优先SSTF(Short Seek Time First):
   1. 计算*队列中各磁头定位位置*,选择**离当前磁头位置最近的**.
   2. 不能保证平均寻到时间最短,**存在饥饿现象**,但比FCFS好.
3. 扫描(SCAN)算法:
   1. 计算**磁头前进方向**上,移动距离最短的请求.一直到最边缘才改变方向。
   2. 没有饥饿.性能较好.
   3. **LOOK**： **前进方向没有请求时就改变方向**.
4. 环扫描算法CSCAN(Circular SCAN)：
   1. 按一个方向前进, 到达最外层磁道并访问后*立刻返回到最里层需要访问的磁道*.相当于从最小磁道号和最大磁道号循环链接.
   2. 中重负荷下效果好.
   3. **C-look**：当前进方向没有请求就返回，返回到有请求的最内磁道。
5. 解决**磁臂粘着**问题:N-Step-SCAN算法
   1. 将队列分为N个的小队列.队列间FCFS,队列内SCAN.N值很大时接近SCAN,N=1时FCFS.
   2. *FSCAN调度算法*:设置N=2,分为两个队列.交替使用SCAN扫描两个队列,当一个队列被扫描时,新的请求放入第二个队列.
6. 如果磁盘替换为随机访问的Flash存储器，是否有比CSCAN更高效的磁盘调度策略？说明理由:
### 磁盘高速缓存
利用*内存空间*暂存从磁盘中读出的盘块信息.**逻辑上属于磁盘,物理上位于内存**.
1. 内存中有两种形式:
   1. 固定大小
   2. 根据IO频繁程度,将未使用的内存分配给分页系统和磁盘.
2. 交付方式:
   1. 进程请求磁盘内容时,os查询高速缓冲;命中直接复制一份给进程;*未命中读取磁盘,并复制一份到高速缓存*.以后可以直接使用.
   2. 交付可以是将数据复制到进程内存区;也可以是将数据指针给到进程.
3. 置换算法
   1. 类似请求调页系统.磁盘数据放入高速缓存时,*若缓存已满则置换.*
   2. 最近最久未用LRU、最近未使用NRU、最少使用算法。
   3. LRU链:将高速缓存形成一个**LRU链**,严重影响数据一致性或可能不再使用的盘块放在队首,快速置换;最近使用的放在队尾.
4. 设计时需要考虑数据一致性:系统故障时,*高速缓存和磁盘数据不一致.*解决:
   1. 备用电源
   2. OS定期回写机制:定期将“dirty”块、内存专用块、活动 i 结点写回外存
### 提高磁盘I/O速度的其它方法
1. *提前读*Read-Ahead:用户读取某个盘块时,将当前盘块和*下一盘块*一起放入高速缓存
2. *延迟写*:用户写入磁盘时,*暂时不写*,放入空闲缓冲区队尾.下次再读时可以快速获取.当缓冲区需要被使用时,再写入磁盘.
3. *优化物理块的分布*:同一个文件占用盘块**尽量分布在同一磁道或相邻磁道**.将同一条磁道上的若干个盘块组成一簇，*以簇为单位给文件分配空间*
4. *虚拟盘*:内存空间仿真磁盘(RAM盘),驱动程序在内存执行,用户透明,标准与普通驱动一致.但有**易失性**,通常用于存储临时文件.
## 做题
1. 一个文件系统可使用的容量**不等于磁盘容量，因为磁盘可以分区，每个分区都有独立文件系统**
2. 一个文件系统可以管理的文件数量受限于**文件控制块**数量
3. 文件存储空间管理是指**外存空闲区管理**
4. 可以用来判断空闲块的方法：
   1. 位图
   2. 空闲（链）表
   3. FAT表（File Allocation Table），记录物理块的下一个块，-1时就是空闲的。
5. 磁盘调度算法中，不会产生**磁臂黏着**的是**FCFS和N-step-Scan**
6. 计算CSCAN等磁道移动数时，**从一侧快速移动到另外一侧的磁道移动数N也要计算**
# 文件管理
提供安全、高效、快速和方便的**信息存储和访问功能**
1. 文件管理的目的:
   1. *方便的文件访问和控制*：以符号名称作为文件标识，便于用户使用
   2. *并发*文件访问和控制：在多道程序系统中支持对文件的并发访问和控制；
   3. 统一接口
   4. 多种文件*访问权限*：不同用户对同一文件有不同的访问权限；**你的角色是什么??你角色在哪里??你只有一个角色不行的**
   5. 优化性能：存储效率、检索性能、读写性能；
   6. 差错恢复：能够验证文件的正确性，并具有一定的差错恢复能力。
2. 文件定义:**一组有意义的信息/数据集合**.文件包含文件体和文件属性.
3. 文件**基本属性**:
   1. 文件名:*同文件夹下*不允许重名文件
   2. 标识符:内部名称,*整个系统中*文件标识符唯一
   3. 类型
   4. 位置:存放路径+外存地址(用户透明)
   5. 大小,创建时间、上次修改时间,文件所有者信息、保护信息
4. *数据项*:
   1. 基本数据项:数据基本单位,原子数据,有基本类型.例如数据库中一个字段:姓名,学号...
   2. 组合数据项:若干基本数据项集合
5. **记录:一组相关数据项集合**.描述一个对象在某方面的属性,主关键字（关键字）用于标识一个记录.
6. 文件:创建者定义，具有文件名的一组相关元素的集合.表示符号是文件名.
   1. 有结构文件:由若干相关记录组成；
   2. 无结构文件:看成字符流。
## 文件系统
管理文件的机构,提供文件存储和访问.
...
![alt text](image-107.png)
   1. *有结构文件*(如数据库表)--由一组一组相似的记录组成，又称“*记录式文件*”
   2. *无结构文件*(如文本文件)--由一些二进制或字符流组成，又称“*流式文件*”
   3. 
2. 对文件的操作:
   1. *创建文件*:分配外存,文件系统中建立目录项,*create系统调用*:检查参数合法性,在文件目录项增加一个FCB,参数填入FCB,分配外存空间,建立索引表填入FCB
   2. *删除文件*:找到对应目录项删除,回收内存,*delete*系统调用:检查参数得到文件名,查找文件目录结构得到FCB,照索引表回收对应内存,删除FCB目录项
   3. *打开文件*:*open*系统调用,
   > 1. 检查参数，获得文件名,查找目录结构找到FCB,调入内存并放入*内存活跃文件目录表*
   > 2. 建立一张本次打开的“文件读写状态信息表”，将当前访问指针指向文件首；
   > 3. 将文件读写状态信息表的地址存于进程PCB中的资源描述区中“打开文件表”中，返回其*索引号fd*作为本次调用的返回值 。
   4. *关闭文件*:close系统调用:
   > 1. 检查参数，获得fd；
   > 2. 按fd在PCB中的“打开文件表”中得到“文件读写状态信息表”的指针，释放其空间，如果“活跃文件目录表”中FCB不再使用，则释放该FCB所占的内存空间。
   5. **什么鬼,这玩意懒得写了**
   ![alt text](image-109.png)
3. os需要提供的**文件基本操作**:
   1. 创建文件create、删除文件delete、读写文件（read）/write（读写前必须打开，读写结束必须关闭。参数中**不包含文件名，而是打开后直接使用文件句柄（handler）进行读写**）、打开关闭文件open/close
   2. 其中，复杂的操作（如复制文件等）可以通过简单操作来完成。
4. 其他文件操作：
   1. 以*系统调用形式给用户*
   2. 文件改名、权限修改、目录的创建/删除、文件共享等
5. *文件在外存中存储*：
   1. 外存中，内存以块为单位，文件必须以块的形式存储。文件逻辑地址表示为* (文件块号，块内偏移)*。os负责将逻辑地址转换为物理地址*(物理块号，块内偏移)*
## 文件的逻辑结构
逻辑结构（*文件的组织*）：用户看来，数据的组织方式，即内部的逻辑结构或用户访问模式。
物理结构：在内存/外存上的
1. 逻辑结构设计要求：
*访问性能*：便于检索；便于修改
*存储性能*：向物理存储转换方便，节省空间
### 无结构文件
无结构文件的数据就是一系列二进制流或字符流，没有逻辑结构问题。
1. *流式文件*：文件体为字节流。不划分记录，顺序访问。每次读写访问可以访问任意长度，没有格式处理
### 有结构文件
由一组*相似的记录*组成，又称“记录式文件”。
1. 组成：每条*记录*由若干个*数据项*组成，有一个*关键字*数据项。
2. 根据每条记录的长度，分为*定长记录和可变长记录*
![alt text](image-111.png)
![alt text](image-112.png)
3. 根据逻辑组织方式，分为*顺序*、*索引*、*索引顺序*文件
   1. **顺序**：数据按照某种顺序排列，适合于*定长文件*
   2. **索引文件**：记录长度可变时，建立一张索引表。每个表项定位一个记录。
   3. **索引顺序文件**：*一组*记录分配一个表项
##### 顺序文件
记录一个接一个地顺序排列(逻辑上)。记录定长或可变长。*物理上顺序/链式存储*。
1. 顺序存储：
   1. **定长可随机存取**（i*L）；变长不可随机存取。
   2. 串结构无法快速找到某关键字；顺序结构可以使用*折半查找*等方式快速找到关键字。
   3. 顺序结构难以增加一条数据。串结构容易
2. 链式存储：无论数据定长或可变长，都不能随机存取，*必须从头开始遍历*。
##### 索引文件
可变长文件无法随机访问。*索引表（本身是顺序的定长文件）来存储块索引，可以很快的找到第i个块，主要用于对信息处理的及时性要求比较高的场合*。
![alt text](image-113.png)
1. 可以*使用不同表项*建立索引表。例如使用学号或姓名来建立。
##### 索引顺序文件
对于索引表，*存储表较为浪费存储空间*。可以采取索引顺序方式，**将记录分组，可以节省空间**
![alt text](image-114.png)
1. 索引表是一个定长记录的串结构文件。
2. 效率分析：
   1. 一个文件10000个记录，平均查找5000次。然而，分100组每组100个后，平均查找50（查组） + 50（查表）=100次。
3. 多级索引顺序文件：建立多级索引表，将记录分组后，组中再次分组，大大降低查询次数。
![alt text](image-115.png)
   1. $N$个记录建立$K$级索引，最优组内记录个数是$N^{\frac{1}{K+1}}$
   2. 检索一个记录的平均查询次数是$(\frac{N^{\frac{1}{K+1}}}{2}) \times (K+1)$
4. 直接文件：根据记录的键值直接获得记录的物理地址.关键在于实现从**键值到物理地址**的转换。
   1. 哈希文件：应用最广泛的直接文件。给出*记录的键值，经过哈希函数可以得到一个文件中的相对位置*。通常是一个目录表项，可以查询到物理地址。**访问速度最快，但在主文件中有空闲空间浪费**。
## 文件目录
目录是*由文件说明组成*的用于文件检索的一个特殊文件,用于标识文件物理地址。
1. 需要解决：按名存取、快速检索：合理组织目录结构、文件共享、文件重名的解决
### 文件控制块FCB
1. 目录文件：![alt text](image-116.png)
2. FCB定义：
   1. 目录文件*存储的每条记录都是一个FCB*。实际上，上图的根目录文件的每个表项也是一个FCB。
   2. FCB实现了文件名和文件的映射，*实现了用户的按名存取*。
   3. FCB包含的。**最重要、最基本的信息是文件名、文件存放的物理地址。**
3. FCB包含信息：
   1. 基本信息类：**文件名**、别名（字符串）；**物理位置**（文件卷、块号、盘块长度）；*逻辑结构*：记录/流式；*物理结构*：顺序、链式、索引式、哈希文件
   2. 存取控制信息类：文件主人/高级用户/普通用户存取权限。
   3. 使用信息类：建立、修改时间、打开的进程数、修改状态等、
4. 对目录进行的*操作*：
   1. 搜索：文件名搜索，找到对应目录项
   2. 创建/删除文件：增删目录项
   3. 显示：显示目录内容。以及其中的文件属性。
   4. 修改:修改文件属性，例如改名
### 目录结构
##### 单级目录结构
早期设计。整个系统只有*一张目录表*。文件不能重名。
创建文件必须先查看文件是否重名。效率低。
##### 两级目录结构
主文件目录 (MFD, Master File Directory) 和用户文件目录 (UFD, User File Directory).
![alt text](image-117.png)
##### 多级目录结构(树型目录)
文件路径名标识文件。
1. *绝对路径*：路径名是个*字符串*。各级目录之间用“/”隔开。从根目录出发的路径称为**绝对路径**。
2. *相对路径*:设置当前目录为*./*，则可以从当前目录出发继续寻找下级目录内容。
3. IO次数：
![alt text](image-118.png)
   1. 绝对路径:找自拍.jpg，首先从外存IO读入根目录表，找到照片（1）；再从照片目录表找到2015-08表，从外存IO读入；最后从2015表IO读入自拍文件。*共三次IO，每次查找新表、新文件都要IO*
   2. 相对路径：再当前目录2015中，*目录表存在内存*。每次仅需要查找内存的文件名，然后IO访存找到对应文件即可。
4. 结构清晰，分类明确，*但不便于实现文件的共享*
##### 无环图目录结构
用不同的文件名指向同一个文件或目录。再树形基础上，增加多个指向同一个节点的**有向边**。
![alt text](image-119.png)
1. 共享计数器：每个共享节点设置，记录多少个地方在共享本节点。*删除节点时，对应共享计数器-1。除非共享计数器变为0，否则不真的删除节点*。
2. 文件修改是全局的，所有共享的用户都能看到。 
### 文件查询技术
**目录查询→FCB（I-node）→物理地址（盘块号）→物理位置（柱面、磁头号、扇区号）。**
1. 索引节点I-node：将FCB拆成*文件目录*（包含名字+索引节点编号）+I节点（文件描述信息+数据结构）
   1. 可以加速*文件名检索*速度.
   2. 精简目录表FCB，FCB中**只存储文件名信息+I-node**，加速检索。而I-node中存储其他信息
   ![alt text](image-120.png)
   3. 加速原因：当目录表项详细时，FCB一项占用内存多（例如64B），此时*一个磁盘块（例如128B）能存储的目录表项少（2个）*。此时查询一个文件（20个表项），必须多次IO访存获取表（20/2=10次）。然而当目录表项只有文件名和I-node指针时（16B），一个磁盘块*存储的目录表项多（8个）*，IO访存次数变少(20/8 = 3次)
   4. 外存中索引结点称为“*磁盘索引结点*”(包含文件主标识、类型、存取权限、物理地址、文件长度、共享技术、存取时间)，文件名找到文件后，IO访存读出I-node放入内存，放入内存后称为“*内存索引节点*”。内存索引节点有额外信息，例如是否修改、访问计数等。
## 文件共享
多个用户（进程）共享同一个文件，系统中只保留一个副本。有单机、多机、网络范围共享。
1. 一个用户修改文件，所有用户都能看到修改。
2. *索引节点的共享方式：硬链接*
   1. 索引节点Inode中存放count计数器，标识多少用户共享了此文件。*删除时count-1,而不真的删除，除非count=0*
3. *符号链接：软链接*:
   1. Link类型文件，记录文件存放路径，类似快捷方式。系统根据Link文件提供的路径来层层访存，找到文件。
   2. 软链接对应的真实文件被删除后，**Link文件仍然存在，但已经找不到这个文件了**
4. 文件共享的*语义*：
   1. 语义定义：读操作得到的是**在其之前所有写操作**的最终值。
   2. 要解决一个用户读同时一个用户写的问题。系统为所有操作维持一个绝对的时间顺序，*读者-写者问题*。
   3. 分布式系统可能存在：服务器文件和客户端Cache不同。
   > 1. 放松文件共享语义：对一个打开文件的修改一开始只对那个修改文件的进程可见。仅当文件被关闭后，修改才为其他进程所见：**会晤语义**(会晤语义结果随机或取决于谁最后关闭文件。)；
   > 2. 或可以完全禁止文件修改，只允许读
   > 3. 原子事务:所有事务操作中的文件调用按照严格顺序执行，不被任何其他并发事务打断
## 文件保护
1. 口令保护:口令存放在FCB或I-node。直接存储口令不安全
2. 加密保护：算法+密钥保护。然而加解密耗时。
3. 访问控制：在每个文件的FCB(或索引结点)中增加一个**控制访问表**(Access-Control List, ACL)，该表
中记录了各个用户可以对该文件执行哪些操作。
![alt text](image-121.png)
   1. ACL存储可能消耗空间大，使用精简表：以组为单位，分为管理员等组，用户属于组来进行访问控制。
   ![alt text](image-122.png)
   2. 只有管理员和超级用户可以创建和修改组
## 做题：
1. 系统级安全管理包括*注册和登录*
2. 逻辑文件存储到存储介质上时，**采用的组织方式和存储介质特性有关**，例如磁带只能顺序存储
3. 一个数据结构是否需要使用数组来实现，**主要看是否有随机存取的需求**
4. 为了用二进制位表示不同用户组的不同权限（完全控制、读、写、执行等），**每一种权限都需1位（若5种权限则需要5位），每个用户都需要。**
